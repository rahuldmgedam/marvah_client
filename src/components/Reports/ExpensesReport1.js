...
export default function CreditReport1({ dbpath1, date2 }) {
  ...
  let query="SELECT * FROM `rwt_daily_expenses` WHERE `date` LIKE '%"+date+"%'";
      /*    alert(query); */
    ...
    <>
      <div className='tankMainDiv shadow-lg p-3 mb-5 bg-body-tertiary rounded'>
        ...
        <div id='printtoPdf'>
          ...
          <br />
          <center><b>Expenses Report - Montlhy</b><br /><br />
            <table>
              <tr>
                <th  className='css_border' style={{ width: '120px' }}>Date</th>
                <th  className='css_border' style={{ width: '120px' }}>Name</th>
                <th  className='css_border' style={{ width: '120px' }}>Amount</th>
                <th  className='css_border' style={{ width: '120px' }}>Narration</th>
               </tr>
              {data.map((res) => (
                <tr key={res.date}>
                  <td  className='css_border'>{convertDateFormat(res.date)}</td>
                  <td  className='css_border'>{res.name}</td>
                  <td  className='css_border'>{res.amount}</td>
                  <td  className='css_border'>{res.narration}</td>
                </tr>
              ))}
            </table>
            </center>
