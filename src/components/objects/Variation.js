...
export default function Variation() {
  ...
  const [msTank1Decant, setMsTank1Decant] = useState(() => localStorage.getItem('msTank1Decant') || null);
  const [speedTank2Decant, setSpeedTank2Decant] = useState(() => localStorage.getItem('speedTank2Decant') || null);
  const [hsdTank3Decant, setHsdTank3Decant] = useState(() => localStorage.getItem('hsdTank3Decant') || null);
  ...
  useEffect(() => {
    localStorage.setItem('msTank1Decant', msTank1Decant);
    localStorage.setItem('speedTank2Decant', speedTank2Decant);
    localStorage.setItem('hsdTank3Decant', hsdTank3Decant);
  }, [msTank1Decant, speedTank2Decant, hsdTank3Decant]);
...
// console.log(Ms,Ms2,HSD)
  ...
  // Calculate total stock
  const calculateTotalStock = (receipt, openStock) => {
    return parseInt(receipt) + parseInt(openStock);
  };
  ...
  <td>
    <input
      type="text"
      className={`${
        variationValue >= 0 ? "form-control bigFontWeight text-center" : "text-red-500 text-center"
      }`}
      style={{ width: "112px" }}
      value={actualBalance ? variationValue : ""}  // Show 0 if Actual Bal Stk. is not entered
      disabled
    />
  </td>
  <td>
    <input
      type="text"
      className={`${
        tVariation >= 0 ? "form-control bigFontWeight text-center" : "text-red-500 text-center"
      }`}
      style={{ width: "112px" }}
      value={actualBalance ? tVariation : ""}  // Show 0 if Actual Bal Stk. is not entered
      disabled
    />
  </td>
</tr>
      ...
      <button
        type="button"
        style={{ height: "30px", marginLeft: "90%" }}
        className={`${isRed?'bg-red-500 text-white px-3 py-1 rounded-md':'bg-green-500 text-white px-3 py-1 rounded-md'}`}
        onClick={handleSave}
      >
        Save
      </button>
