...
export default function Petrol_Invoice_Feeding() {
  ...
  const calculateValues = (formData, filterData) => {
    ...
    return {
      ...
      totalAmount: parseFloat(productAmount)+ parseFloat(vatlst*vat)+parseFloat(cess * klQty),
          ...
          <table className="">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className="border-2 border-gray-900"> {(totalSum).toFixed(2)}</td>
              </tr>
            </tbody>
          </table>
        </div>
              ...
              {petrolInvoice.length > 0 &&
                petrolInvoice.map((item, index) => (
                  <tr key={index} className="text-center">
                    ...
                    <td className="border-2 border-gray-900">
                      {(item.klQty * item.lfrPerKl).toFixed(2)}
                    </td>
                    <td className="border-2 border-gray-900 w-[16]">
                      {item.cgst}
                    </td>
                    <td className="border-2 border-gray-900 w-[16]">
                      {(item.klQty * item.lfrPerKl).toFixed(2) * item.cgst}
                    </td>
                    <td className="border-2 border-gray-900">{item.sgst}</td>
                    <td className="border-2 border-gray-900 w-[16]">
                      {(item.klQty * item.lfrPerKl).toFixed(2) * item.sgst}
                    </td>
                    <td className="border-2 border-gray-900 w-[12]">
                      {(
                        item.klQty * item.lfrPerKl +
                        (item.klQty * item.lfrPerKl).toFixed(2) * item.sgst * 2
                      ).toFixed(2)}
                    </td>
